    ...
    Mono<String> sendEmailNotifcation(Long fspPolicyId, Policy policyReq) {
        return customerRepository.getCustomerEmailDetailsByPolicyId(fspPolicyId)
                .flatMap(customer -> {
                    ...
                    return emailService.sendFSPInsuranceWelcomeEmail(customer, "Welcome to eSure Cover Insurance – We're Excited to Have You!")
                            .flatMap(email-> emailService.sendFSPInsuranceWelcomeEmail(customer,"Welcome to eSure Cover Insurance – We're Excited to Have You!")
                                        .flatMap(Mono::just)
                            );
                }).onErrorResume(err -> {
