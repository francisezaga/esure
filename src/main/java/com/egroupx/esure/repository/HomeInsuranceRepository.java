...
public interface HomeInsuranceRepository extends ReactiveCrudRepository<Building,Long> {
    ...
    @Query("INSERT IGNORE INTO esure_buildings SET fsp_quote_ref_id=:fspQouteRefId,description=:description,cover_type=:coverType,construction=:construction,roof_construction=:roofConstruction,sum_insured=:sumInsured,recent_loss_count=:recentLossCount,property_owned_claim=:propertyOwnedclaim")
    ...
    @Query("INSERT IGNORE INTO esure_household_sh_link SET fsp_quote_ref_id=:fspQouteRefId,link_type_id=:linkTypeId,fsp_sh_link_ref_id=:fspShLinkRefId")
    Mono<SHLink> saveHouseHoldShLinkDetails(Long fspQouteRefId,String linkTypeId,Long fspShLinkRefId);

    @Query("select * from esure_buildings where fsp_quote_ref_id=:fspQuoteRef")
    Flux<Building> getBuildingByQuoteRefId(Long fspQuoteRef);

    @Query("select * from esure_buildings_sh_link where fsp_quote_ref_id=:fspQuoteRef")
    Flux<SHLink>  getBuildingShLinkByQuoteRefId(Long fspQuoteRef);

    @Query("select * from esure_household_contents where fsp_quote_ref_id=:fspQuoteRef")
    Flux<HouseHoldContents>  getHouseHoldContentsByQuoteRefId(Long fspQuoteRef);

    @Query("select * from esure_household_sh_link where fsp_quote_ref_id=:fspQuoteRef")
